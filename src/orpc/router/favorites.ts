...
export const getFavorites = os
  ...
  .handler(async ({ context }) => {
    ...
    const result = await db
      .prepare('SELECT * FROM favorite WHERE userId = ? ORDER BY addedAt DESC')
      .bind(userId)
      .all<D1Favorite>()
...
export const removeFavorite = os
  .input(
    z.object({
      foodId: z.string(),
    }),
